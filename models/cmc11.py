...
# identical to cmc10 model except different ast parameters
...
class Cmc11Model:
    def __init__(self, config):
        ...
        config['3dsmls'] = True 
        config['smlmaxchunks'] = 10
        config['smlchunklen'] = 100
    ...
    def create_model(self):
        ...
        sml_input = Input(shape=(self.config['smlmaxchunks'],self.config['smlchunklen']))
        
        tdel = Embedding(output_dim=self.embdims, input_dim=self.tdatvocabsize, mask_zero=False)
        tde = tdel(tdat_input)
        se = TimeDistributed(Embedding(output_dim=self.smldims, input_dim=self.smlvocabsize, mask_zero=False))(sml_input)

        se_enc = TimeDistributed(CuDNNGRU(self.recdims))
        seout = se_enc(se)
